    ...
    if os.path.isfile(fpath):
      ...
      for env_name in env_names:
        try:
          ...
          running_tags[env_name] = input(f'Error retrieving tag {env_name}, input manually: ').strip()
...
def get_docker_tag(env_name: str) -> str:
  ...
  try:
    ns, type, name, container_no = [v.lower().replace('_', '-') for v in env_name.split('__')[1:]]
  except:
